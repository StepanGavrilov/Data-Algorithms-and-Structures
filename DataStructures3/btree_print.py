...
def print_btree(node, nodeInfo=None, inverted=False, isTop=True):
    ...
    leftTextBlock = [] if not leftNode else print_btree(leftNode, nodeInfo, inverted, False)

    rightTextBlock = [] if not rightNode else print_btree(rightNode, nodeInfo, inverted, False)
